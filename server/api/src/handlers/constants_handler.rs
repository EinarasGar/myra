...
use crate::{
    app_error::AppError, states::{TransactionServiceState}, view_models::{transaction_view_model::CategoryViewModel},
};
...
pub async fn get_categories(
    ...
    let ret_vec: Vec<CategoryViewModel> = categories_vec.iter().map(|val| val.clone().into()).collect();
    Ok(ret_vec.into())
}