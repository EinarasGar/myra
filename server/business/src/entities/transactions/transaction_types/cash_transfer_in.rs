...
mod tests {
    ...
    #[test]
    fn try_from_dto_stamps_reserved_cash_transfer_in_category() {
        crate::test_support::load_dynamic_enums();
        let account_id = Uuid::new_v4();
        let dto = transfer_in_dto(account_id, dec!(100), vec![dec!(-9)]);

        let transaction = CashTransferInTransaction::try_from_dto(dto, Uuid::new_v4())
        ...
        assert_eq!(
            ...
            .expect("cash transfer in category id")
        );
    ...
    #[test]
    fn try_into_dto_round_trips_positive_leg_and_fee_entries() {
        crate::test_support::load_dynamic_enums();
        let account_id = Uuid::new_v4();
        let dto = transfer_in_dto(account_id, dec!(100), vec![dec!(-9)]);
        ...
        match round_trip.transaction_type {
            TransactionTypeDto::CashTransferIn(metadata) => {
            ...
            _ => panic!("Expected cash transfer in metadata"),
        }

        assert_eq!(round_trip.fee_entries.len(), 1);
        assert_eq!(round_trip.fee_entries[0].entry.quantity, dec!(-9));
        assert_eq!(
    ...
    #[test]
    fn get_portfolio_action_raises_cash_net_of_fees() {
        crate::test_support::load_dynamic_enums();
        let account_id = Uuid::new_v4();
        let dto = transfer_in_dto(account_id, dec!(100), vec![dec!(-9)]);
        ...
        assert_eq!(cash.units(), dec!(91));
        assert_eq!(cash.fees(), dec!(9));
    }
}
