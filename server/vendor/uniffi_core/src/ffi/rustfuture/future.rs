...
impl<FfiType> RustFuture<FfiType> {
    ...
    pub(super) fn poll(self: Arc<Self>, callback: RustFutureContinuationCallback, data: u64) {
        ...
        if ready {
            ...
            let continuation = self.scheduler.lock().unwrap().store(callback, data);
    ...
    pub(super) fn cancel(&self) {
        let continuation = self.scheduler.lock().unwrap().cancel();
        if let Some(continuation) = continuation {
            continuation.invoke();
        }
    }
    ...
    Scheduler: Send + Sync,
{
    ...
    unsafe fn waker_wake(ptr: *const ()) {
        ...
        let continuation = Self::recreate_arc(ptr).scheduler.lock().unwrap().wake();
        if let Some(continuation) = continuation {
            continuation.invoke();
        }
    }
    ...
    unsafe fn waker_wake_by_ref(ptr: *const ()) {
        ...
        let continuation = (*ptr).scheduler.lock().unwrap().wake();
        if let Some(continuation) = continuation {
            continuation.invoke();
        }
    }
