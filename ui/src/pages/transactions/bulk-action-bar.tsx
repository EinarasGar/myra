...
export function BulkActionBar({ onDeleteSelected, onGroupSelected }: BulkActionBarProps) {
  const { selectedItems, exitSelectionMode } = useTransactionSelectionStore();
  const count = selectedItems.size;
  const allIndividual = Array.from(selectedItems.values()).every(t => t === 'individual');

  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-muted/50 border rounded-md">
      <span className="text-sm font-medium">{count} selected</span>
      <div className="flex-1" />
      <Button variant="outline" size="sm" onClick={onGroupSelected} disabled={!allIndividual || count < 2}>
