...
import type { CombinedTransactionItem, IdentifiableTransaction } from '@/api/api';
...
export function GroupSelectedDialog({
  ...
  const handleSubmit = () => {
    ...
    const selectedTransactions: IdentifiableTransaction[] =
      findTransactionsByIds(allItems, selectedTransactionIds) as IdentifiableTransaction[];
  ...
  const canSubmit = selectedDate && selectedCategory && selectedTransactionIds.length > 0;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!canSubmit || groupTransactions.isPending}>
