...
interface TransactionSelectionState {
  ...
  enterSelectionMode: (initialId?: string, type?: 'individual' | 'group') => void;
...
export const useTransactionSelectionStore = create<TransactionSelectionState>((set) => ({
  ...
  enterSelectionMode: (initialId, type) => set((state) => {
  ...
  exitSelectionMode: () => set({ isSelecting: false, selectedItems: new Map() }),
  toggleItem: (id, type) => set((state) => {
    const items = new Map(state.selectedItems);
